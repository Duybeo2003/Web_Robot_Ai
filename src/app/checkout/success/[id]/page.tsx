...
export default async function CheckoutSuccessPage({
  ...
  const whereClause: Prisma.OrderWhereInput = { id: resolvedParams.id };
  if (session?.user?.id) {
    ...
    <div className="container mx-auto px-4 py-16 bg-[#F5F5F5] min-h-[70vh] flex items-center justify-center">
      <div className="bg-white p-8 md:p-12 rounded-sm shadow-xl shadow-green-500/10 border border-green-100 max-w-2xl w-full text-center relative overflow-hidden">
        ...
        <p className="text-neutral-600 mb-8 max-w-md mx-auto">
          Cảm ơn bạn đã mua sắm tại RoboEQ. Đơn hàng của bạn đang được xử lý và
        ...
        {order.paymentMethod === "BANK_TRANSFER" && (
          <div className="bg-orange-50 border border-orange-100 p-6 rounded-sm mb-8 text-left">
            <h2 className="text-lg font-bold text-orange-800 mb-4 text-center">
            ...
            <div className="flex flex-col md:flex-row items-center justify-center gap-8">
              ...
              <div className="bg-white p-4 rounded-xl shadow-sm border border-orange-200">
                ...
                <img
                  src={`https://img.vietqr.io/image/vcb-1058744697-compact2.png?amount=${order.totalAmount}&addInfo=RoboEQ%20${order.id.toUpperCase()}&accountName=NGUYEN%20QUOC%20DUY`}
                  alt="VietQR Payment"
            ...
            <p className="text-sm text-orange-700 mt-6 text-center font-medium bg-orange-100/50 p-3 rounded-sm border border-orange-100">
              Đơn hàng sẽ được xử lý sau khi chúng tôi nhận được thanh toán. Bạn
              có thể chụp lại màn hình này để chuyển khoản sau.
            </p>
          </div>
        )}
        ...
        <div className="flex flex-col sm:flex-row gap-4 justify-center">
          <Link href="/profile/orders">
            <Button
              variant="outline"
              className="w-full sm:w-auto h-12 px-8 border-neutral-200 text-neutral-700"
            >
