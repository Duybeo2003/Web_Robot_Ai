      ...
      <div className="px-4 py-6 border-b border-neutral-100 bg-white">
        <div className="relative flex justify-between">
          ...
          <div
            ...
            style={{
              width: `${currentIndex > 0 ? (currentIndex / (steps.length - 1)) * 80 : 0}%`,
            }}
          ></div>
          ...
          {steps.map((step, idx) => {
            ...
            return (
              ...
              >
                <div
                  className={`w-8 h-8 rounded-full flex items-center justify-center transition-colors duration-300 ${isActive ? "bg-green-500 text-white shadow-md shadow-green-500/20" : "bg-neutral-200 text-neutral-400"}`}
                >
                  <Icon className="w-4 h-4" />
                </div>
                <span
                  className={`text-[11px] md:text-xs font-semibold text-center ${isActive ? "text-green-700" : "text-neutral-400"}`}
                >
                  {step.label}
                </span>
              </div>
      ...
      {orders.length === 0 ? (
        ...
        <div className="space-y-6">
          {orders.map((order) => (
            <div
              key={order.id}
              className="border border-neutral-200 rounded-sm overflow-hidden shadow-sm hover:shadow-md transition-shadow bg-white"
            >
              <div className="bg-neutral-50 px-4 py-3 flex items-center justify-between border-b border-neutral-200 text-sm">
                <div className="flex flex-wrap items-center gap-4 text-neutral-600">
                  <span className="font-bold text-foreground uppercase tracking-wider text-xs bg-white px-2 py-1 border border-neutral-200 rounded-sm">
