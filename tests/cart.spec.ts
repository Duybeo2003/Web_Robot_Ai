...
test.describe("E-commerce Shopping Flow", () => {
  test("should load products from database and open Cart Sheet when adding to cart", async ({
    page,
  }) => {
    ...
    const cartItemTitle = page.locator(".sm\\:max-w-md a", {
    ...
    const quantityText = page
      .locator(".sm\\:max-w-md span", { hasText: "1" })
      .first();
