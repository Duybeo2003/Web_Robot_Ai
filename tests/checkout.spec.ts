...
test.describe("E-commerce Checkout Flow", () => {
  ...
  test("should complete the checkout flow using BANK_TRANSFER", async ({
    ...
    const otpSwitchBtn = page.locator("button", {
      hasText: "Đăng nhập bằng OTP (SĐT)",
    });
    ...
    await Promise.all([page.waitForNavigation(), verifyBtn.click()]);

    // 8. After login, the page reloads or auth state updates
    try {
      await expect(page.locator("h1", { hasText: "Thanh toán" })).toBeVisible({
        timeout: 10000,
      });
    } catch (e) {
      console.log("Failed to find Thanh toán. Current URL:", page.url());
      console.log(
        "Body text after reload:",
        await page.locator("body").innerText(),
      );
      const ls = await page.evaluate(() =>
        window.localStorage.getItem("roboed-cart"),
      );
